...
from field import UniformNonPeriodicGrid, LinearOperator
...
class Left(BoundaryCondition):
    ...
    def operate(self):
        slice = axslice(self.axis, 1, None)
        BC = self.value - apply_matrix(self.matrix[:,1:], self.field.data[slice], self.axis)
        BC /= self.matrix[0,0]
        slice = axslice(self.axis, 0, 1)
        self.field.data[slice] = BC
...
class Right(BoundaryCondition):
    ...
    def operate(self, out=None):
        slice = axslice(self.axis, None, -1)
        BC = self.value - apply_matrix(self.matrix[:,:-1], self.field.data[slice], self.axis)
        BC /= self.matrix[0,-1]
        slice = axslice(self.axis, -1, None)
        self.field.data[slice] = BC
