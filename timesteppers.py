...
class FullyImplicitTimestepper(ExplicitTimestepper):
    ...
    def __init__(self, eq_set, tol=1e-5):
        ...
        self.tol = tol
        self.J = eq_set.J
        self.N = np.prod(self.X.data.shape)
...
class BackwardEulerFI(FullyImplicitTimestepper):
    ...
    def _step(self, dt):
        ...
        self._evaluate_F()
        self.X.data = self.X.data.reshape(self.N)
        self.F.data = self.F.data.reshape(self.N)
        RHS = self.M @ self.X.data
        LHS = self.LHS_matrix @ self.X.data - dt * self.F.data
        residual = LHS - RHS
        i_loop = 0
        while np.max(np.abs(residual)) > self.tol:
            jac = self.M + dt*self.L - dt*self.J()
...
class CrankNicolsonFI(FullyImplicitTimestepper):
    ...
    def _step(self, dt):
        ...
        self.X.data = self.X.data.reshape(self.N)
        self.F.data = self.F.data.reshape(self.N)
        RHS = (self.M - dt/2 *self.L) @ self.X.data + dt/2 *self.F.data
        LHS = self.LHS_matrix @ self.X.data - dt/2 * self.F.data
        residual = LHS - RHS
        i_loop = 0
        while np.max(np.abs(residual)) > self.tol:
            jac = self.M + dt/2 * self.L - dt/2 * self.J()
            dX = spla.spsolve(jac, -residual)
            self.X.data += dX
            self._evaluate_F()
            LHS = self.LHS_matrix @ self.X.data - dt/2 * self.F.data
